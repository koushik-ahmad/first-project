...
const createEnrolledCourseIntoDB = async (
  ...
  try {
    ...
    const maxCapacity = isOfferedCourseExits.maxCapacity;
...
const updateEnrolledCourseMarksIntoDB = async (
  ...
  const modifiedDate: Record<string, unknown> = {
    ...courseMarks,
  };
  ...
  if (courseMarks?.finalTerm) {
    ...
    modifiedDate.grade = result.grade;
    modifiedDate.gradePoints = result.gradePoints;
    modifiedDate.isCompleted = true;
  }

  if (courseMarks && Object.keys(courseMarks).length) {
    for (const [key, value] of Object.entries(courseMarks)) {
      modifiedDate[`courseMarks.${key}`] = value;
    }
  }
